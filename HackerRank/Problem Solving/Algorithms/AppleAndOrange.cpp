...
bool landedOnHouse(int s, int t, int location){
    return location >= s && location <= t;
}

void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int total_apples = 0, total_oranges = 0;
    int length = max(apples.size(), oranges.size());
    
    for(int i = 0; i<length; ++i){
        if(apples.size() > i){
            if(landedOnHouse(s, t, a+apples[i])) total_apples += 1;   
        }
        if(oranges.size() > i){
            if(landedOnHouse(s, t, b+oranges[i])) total_oranges += 1; 
        }
    }
    cout << total_apples << '\n' << total_oranges;
}
